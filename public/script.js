...
$(document).ready(function () {
    ...
    $(document).on('click', '#show-iframe', function () {
        ...
        setTimeout(() => {
            $('.start-page').html('<img class="loading" src="./img/loading.gif" style="display:none"/><br><br><span class="warning">페이지 로딩 후 카카오톡 창이 표시될 때까지 잠시 기다려 주세요</span>'); 
            $('.loading').fadeIn(300);
        }, 300);
        ...
        $.ajax({
            ...
            success: function (data) {
                session_num = data;
                setTimeout(function () {
                    $('main').append(`<iframe src="/${session_num}" frameborder="0"></iframe>`);
